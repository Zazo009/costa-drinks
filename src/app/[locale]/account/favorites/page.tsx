  ...
  return (
    <main className="min-h-screen bg-cream">
      ...
      <section className="mx-auto max-w-5xl px-6 py-10">
        <h1 className="mb-6 text-2xl font-bold text-ink">{t('favoritesTitle')}</h1>

        {favoriteProducts.length === 0 ? (
          <div className="flex flex-col items-center gap-3 rounded-xl border border-dashed border-ink/10 py-16 text-center">
            <Heart className="text-ink/35" size={24} />
            <p className="text-ink/50">{t('noFavorites')}</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {favoriteProducts.map((product) => {
              ...
              return (
                <div
                  key={product.id}
                  className="group flex flex-col rounded-xl border border-ink/[0.06] p-4 shadow-sm"
                >
                  ...
                  <p className="mb-3 mt-1 font-semibold text-ink">
